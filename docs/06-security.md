...
## Auditoría — Endurecimiento Pre-Lanzamiento: Rate Limiting, Credenciales del Worker, Sandboxing y Mitigación Básica de Sybil — 2026-07-09

**Auditor:** Security Agent
**Alcance:** cierre (parcial) de SEC-21, SEC-20, SEC-29 y SEC-23, según el diseño de `docs/04-arquitectura.md` §15. Código verificado directamente, no solo el resumen del ciclo: `backend/app/core/rate_limit.py`, `backend/app/routers/auth.py`, `backend/app/routers/work.py`, `backend/app/worker/main.py`, `backend/app/worker/sandbox.py`, `backend/app/worker/run_workers.sh`, `backend/app/models/auth.py`, `backend/app/db/queries/compute_queries.py` (interacción con SEC-36), `migrations/008_rate_limiting.sql`, `backend/tests/test_rate_limit.py`, `backend/tests/test_worker.py`.

**Confirmación de implementación:** el código coincide con el diseño de §15 sin desviaciones — los 4 endpoints (`/auth/login`, `/auth/register`, `/work/claim`, `/work/{chunk_id}/submit`) usan `check_rate_limit` respaldado por Postgres (no memoria de proceso), las credenciales del worker se resuelven por `CC_WORKER_EMAIL`/`CC_WORKER_PASSWORD` con `--email`/`--password` deprecados, `process_chunk` delega en `run_chunk_sandboxed` (subproceso `spawn` con `RLIMIT_CPU`/`RLIMIT_AS`/timeout de pared), y `RegisterRequest`/`LoginRequest` normalizan el email a minúsculas. Hay tests nuevos (`test_rate_limit.py`, `test_worker.py`) que cubren la lógica unitaria correctamente.

### 1. Rate limiting — ¿es efectivo contra un atacante decidido?

**Lo que SÍ resuelve, con evidencia de código:**

- **Monopolización por un único actor en bucle cerrado (el escenario original de SEC-21):** confirmado bloqueado. `rate_limit_by_provider("work_claim", limit=30, window_seconds=60)` (`backend/app/routers/work.py` líneas 25, 32) hace inviable que una sola cuenta reclame chunks en un bucle cerrado sin límite — el escenario textual del hallazgo original ("un proveedor lanza 100 peticiones concurrentes") ya no funciona: la petición 31 en la misma ventana de 60s recibe 429.
- **Fuerza bruta/credential stuffing de un único origen contra `/auth/login`:** confirmado mitigado. 10 intentos/60s/IP hace que un ataque de diccionario sostenido desde una sola IP sea impracticable a la velocidad necesaria para tener éxito contra bcrypt.
- **Creación masiva de cuentas desde un único origen:** confirmado mitigado a 5/hora/IP.
- **El problema de `--workers 2` que habría roto un rate limit ingenuo en memoria:** correctamente identificado y evitado — el contador vive en `rate_limit_counters` (Postgres), compartido entre los dos procesos Uvicorn. Verificado leyendo `check_rate_limit` (`rate_limit.py` líneas 29-65): el `UPSERT ... RETURNING request_count` es atómico a nivel de fila en Postgres independientemente de qué proceso Uvicorn ejecute la petición.

**Lo que NO resuelve — y el propio diseño ya lo admite explícitamente en §15.4.3, confirmado aquí con evidencia adicional:**

- **Rotación de IP evade por completo el límite de `/auth/register` (5/hora/IP) y el de `/auth/login` (10/60s/IP).** Ambos limitadores usan `_client_ip(request)` (`rate_limit.py` línea 68-75) como única identidad — un atacante con acceso a un pool de proxies, VPN rotativa o tethering móvil con reasignación de IP no encuentra ninguna fricción real más allá de gestionar más orígenes. Esto es honesto en el propio diseño (§15.4.3: "no encuentra ninguna fricción real") y se confirma sin matices al leer el código: no hay ningún componente adicional (huella de dispositivo, CAPTCHA, verificación de email) que añada fricción más allá de la IP.
- **El límite de 30/60s por `provider_id` en `/work/claim` NO limita el volumen total del sistema si el atacante puede crear cuentas nuevas — y crear una cuenta nueva sigue siendo gratis y solo limitado por IP de registro, no por identidad real.** Este es exactamente el punto que pide auditar el encargo, y la respuesta es que la mitigación es **incompleta por diseño, no por error de implementación**: con `max_chunks` hasta 10 (`ClaimRequest.max_chunks`) y 30 llamadas/min, una sola cuenta autenticada puede reclamar hasta 300 chunks/minuto (matemática que el propio §15.1.5 reconoce explícitamente como el límite superior aceptado para el caso legítimo). Nada impide que el mismo atacante repita esto con una segunda, tercera, o Nª cuenta — cada `provider_id` nuevo obtiene su propio balde de 30/60s independiente, porque `check_rate_limit` usa `f"{scope}:provider:{provider['id']}"` (`rate_limit.py` línea 93) como clave, sin ningún componente que agregue por IP, huella de hardware, o cualquier otra señal de "misma entidad real". El único freno al ritmo de creación de esas cuentas es el rate limit de `/auth/register` (5/hora/IP) — bypasseable por rotación de IP como se describe arriba. **Conclusión concreta: un atacante con, por ejemplo, 4 IPs distintas puede tener hasta 20 cuentas activas en la primera hora (5 por IP) y, a partir de ahí, hasta 20 × 300 = 6000 chunks/minuto de capacidad agregada de reclamo — muy por encima de lo que el límite por cuenta pretendía acotar.** Esto no es una interacción rara ni un bug: es la consecuencia directa, ya anticipada por el propio diseño (§15.4.3, "el problema de fondo... sigue exactamente igual de vigente"), de que el rate limiting de `/work/claim` está anclado a una identidad (`provider_id`) que no cuesta nada duplicar.
- **Verificación puntual de que el 429 no tiene ninguna vía de bypass a nivel de aplicación:** confirmado que no la hay — no hay ningún parámetro, cabecera, ni condición en el código que exima a una petición del incremento del contador (a diferencia de, por ejemplo, un `X-Forwarded-For` mal manejado que permitiera falsificar la IP; no se usa ningún encabezado de proxy para `_client_ip`, solo `request.client.host`, así que no hay spoofing de IP vía cabecera — pero si el backend corre detrás de un proxy real sin `uvicorn --proxy-headers` configurado correctamente, **todas** las peticiones aparecerían con la misma IP del proxy, lo que tendría el efecto contrario: bloquear a usuarios legítimos compartiendo esa IP en vez de fallar abierto. El propio código lo señala en un comentario (`rate_limit.py` líneas 819-824) pero es responsabilidad operativa de DevOps verificarlo en el despliegue real, no verificado en este ciclo por no ser un cambio de código).

**Veredicto honesto:** el rate limiting SÍ es efectivo contra el atacante no sofisticado (script simple, una IP, una cuenta) que motivó el hallazgo original. NO es efectivo contra un atacante mínimamente organizado (varias IPs, varias cuentas) — y explícitamente no pretende serlo, según el propio alcance declarado del ciclo. Es una mejora real y sustancial, no un placebo, pero no cierra SEC-21 en su forma más fuerte (ver actualización de estado más abajo).

### 2. Sandboxing — qué contiene y qué NO contiene

**Confirmado que SÍ contiene, con evidencia de código y de test:**

- **Tiempo:** `proc.join(timeout=CHUNK_TIMEOUT_SECONDS)` (`sandbox.py` línea 80) seguido de `terminate()`/`kill()` si el proceso sigue vivo (líneas 83-90). Verificado por `test_run_chunk_sandboxed_aborts_cleanly_on_timeout` (`test_worker.py` líneas 121-147), que reduce el timeout a 0 y confirma que el subproceso se aborta limpiamente y el llamador recibe un dict de error, no una excepción — el proceso worker principal sobrevive. Esto es una prueba real de comportamiento, no solo de lectura de código.
- **CPU y memoria (solo POSIX/Linux, que es el entorno real de despliegue):** `resource.setrlimit(RLIMIT_CPU, ...)` y `resource.setrlimit(RLIMIT_AS, ...)` (`sandbox.py` líneas 41-45) se aplican dentro del propio subproceso hijo antes de invocar el plugin. El código maneja correctamente el caso de que el hijo termine por señal (`proc.exitcode != 0` → error genérico, línea 92-95) — un `SIGKILL` por exceso de `RLIMIT_AS` o un `SIGXCPU` por `RLIMIT_CPU` no se propagan como excepción Python sino como código de salida anómalo, y el código lo captura sin dejar que la excepción escape al proceso padre. **No hay test que ejercite el límite de memoria o CPU real** (a diferencia del timeout, que sí tiene test) — `test_run_chunk_sandboxed_success`/`_unsupported_job_type`/`_returns_tuple_shape` no fuerzan un chunk que realmente agote RLIMIT_AS o RLIMIT_CPU. Esto es una omisión razonable (difícil de hacer determinista y rápido en un test unitario) pero significa que el comportamiento ante agotamiento real de memoria/CPU está verificado por lectura de código y diseño, no por ejecución observada — igual de razonable que confiar en la semántica documentada de POSIX/`resource`, pero vale la pena dejarlo explícito.

**Confirmado que NO contiene — ninguno de estos tres puntos es nuevo respecto a lo que ya declaraba §15.3.5, pero se verifica aquí con más detalle técnico del que el diseño explicitó:**

- **Red: sin ninguna restricción.** El subproceso `spawn` (`sandbox.py` línea 74) es un proceso nuevo del mismo host, mismo namespace de red que el worker padre — puede abrir sockets salientes a cualquier destino sin restricción alguna (no hay `--net=none`, no hay firewall de egress, no hay proxy obligatorio). Si un futuro plugin (o una vulnerabilidad en una dependencia como `polars`/`pyarrow` que permitiera ejecución de código) lograra ejecutar código arbitrario dentro del subproceso, ese código podría exfiltrar datos a un servidor externo, participar en un botnet, o hacer reconocimiento de la red interna del host — exactamente como ya advertía §15.3.5 al decir que esto queda fuera de alcance.
- **Filesystem: sin ninguna restricción.** El subproceso corre con el mismo usuario del sistema operativo que el worker — mismos permisos de lectura/escritura sobre todo el filesystem accesible a ese usuario (código fuente del propio worker, cualquier archivo de configuración, claves SSH del usuario si las hubiera, etc.). No hay `chroot`, bind mount de solo lectura, ni ningún mecanismo equivalente.
- **Variables de entorno del proceso padre: heredadas íntegramente por el subproceso — este es un hallazgo concreto no mencionado explícitamente en §15.3, ver SEC-37 más abajo.** `ctx.Process(target=_child_entrypoint, args=(...))` (`sandbox.py` línea 78) no pasa ningún parámetro `env=` ni limpia `os.environ` antes de lanzar el hijo — el contexto `spawn` de `multiprocessing` en POSIX ejecuta el nuevo intérprete Python heredando el entorno completo del proceso que lo lanza. Esto significa que **`CC_WORKER_PASSWORD` y `CC_WORKER_EMAIL` están disponibles dentro del propio subproceso "aislado"**, pese a que el plugin no los necesita en absoluto (la autenticación ya ocurrió en `login()` antes de que exista ningún chunk que procesar). Es una superficie de exposición innecesaria y evitable: si el objetivo del sandbox es contener un futuro plugin comprometido, dejarle acceso directo a las credenciales del propio worker anula parcialmente el propósito del aislamiento para ese vector específico. Ver SEC-37.
- **Sin límite de número de procesos/hilos (`RLIMIT_NPROC`) dentro del presupuesto de CPU/tiempo del hijo.** Un subproceso comprometido podría, dentro de sus 25s de CPU y 30s de pared, intentar un fork-bomb u otra forma de agotamiento de recursos del host compartido (afectando a otros workers o servicios colocados en la misma máquina) antes de ser terminado por timeout. Menor que los puntos anteriores, pero es otra dimensión de "sin namespaces de SO" que el diseño agrupa genéricamente bajo "no hay aislamiento completo" sin desglosar.
- **Terminación no garantiza la limpieza de procesos nietos.** `proc.terminate()`/`proc.kill()` (`sandbox.py` líneas 85-89) actúan sobre el PID directo del subproceso `multiprocessing`, no sobre un grupo de procesos. Si el código que corre dentro del hijo llegara a lanzar (`subprocess.Popen`, doble fork, etc.) un proceso adicional antes de ser matado, ese proceso nieto NO es hijo directo del objeto `Process` que se está terminando y sobreviviría a la señal — quedaría huérfano, reparentado a `init`/`systemd`, corriendo indefinidamente fuera del control del worker. Es un escenario que requiere que el propio plugin invoque un subproceso (lo que hoy no ocurre — `data_processing.py` no usa `subprocess` en ningún punto, confirmado por lectura completa del archivo), por lo que el riesgo actual es bajo, pero es precisamente el escenario que §15.3.5 identifica como el disparador para exigir contenedores completos ("el momento en que se añada el primer plugin que invoque un binario externo") — vale la pena que quien diseñe ese futuro plugin sepa que el mecanismo de terminación actual no es robusto frente a un proceso que intente sobrevivir deliberadamente a su padre.

**Nuevo hallazgo — SEC-38 (verificación pendiente, no confirmado como explotable, ver abajo):** el límite `CHUNK_MEMORY_LIMIT_MB = 512` se aplica vía `RLIMIT_AS` (memoria virtual direccionable), no memoria residente. Los allocators usados por Rust/Arrow (base de `polars`) son conocidos en general por reservar regiones de memoria virtual más grandes de lo que el conjunto de datos activo requeriría, incluso si esa memoria no llega a comprometerse físicamente. No se ejecutó el plugin real bajo el límite de 512 MB en este ciclo de auditoría (no se dispone de un entorno Linux con `polars` instalado para reproducirlo) — no puedo confirmar si el límite es demasiado ajustado en la práctica. **Se documenta como riesgo de verificación pendiente, no como hallazgo confirmado**: si en producción `RLIMIT_AS=512MB` provoca fallos de chunks legítimos por reserva de memoria virtual (no por uso real), la respuesta más probable bajo presión de incidentes sería subir el límite drásticamente o eliminarlo — lo que debilitaría silenciosamente la única contención de memoria que este ciclo introduce. Recomendación: verificar empíricamente contra el `Dockerfile` de despliegue real antes de confiar en el valor de 512 MB, idealmente con un chunk de tamaño máximo realista (`CHUNK_SIZE` de `compute_service.py`).

### 3. Credenciales del worker — ¿queda alguna forma residual de fuga?

- **`ps aux` / `/proc/<pid>/cmdline`:** resuelto para el flujo recomendado (variables de entorno) — confirmado que ni `CC_WORKER_EMAIL` ni `CC_WORKER_PASSWORD` aparecen nunca como argumento de línea de comandos en el camino recomendado. **Sigue vigente sin cambios para quien todavía use `--password`** (camino deprecado, no eliminado — `main.py` líneas 199-208): el propio diseño lo admite (compatibilidad hacia atrás deliberada), y el `logger.warning` de deprecación (líneas 173-178) es la única mitigación — no hay ningún mecanismo que fuerce la migración, solo lo advierte. Confirmado en código: `run_workers.sh` ya no pasa `--password` en ningún caso (línea 19-30), así que el vector solo persiste para scripts de despliegue externos al repo que el equipo no controla directamente.
- **`/proc/<pid>/environ` en Linux:** este archivo es legible por el propio usuario del proceso y por root, pero no por otros usuarios sin privilegios del mismo sistema (a diferencia de `/proc/<pid>/cmdline`, que es de lectura pública). Migrar a variables de entorno reduce genuinamente la superficie de exposición frente al vector original de SEC-20 (cualquier usuario del sistema vía `ps aux`), pero **no la elimina para dos escenarios concretos**: (a) cualquier proceso que corra como el mismo usuario del sistema operativo que el worker — incluyendo, de forma directa y confirmada en este ciclo, **el propio subproceso "sandboxed" de §15.3** (ver SEC-37 arriba: hereda el entorno completo, así que un plugin comprometido no necesita ni `/proc/<pid>/environ` — ya tiene la contraseña en su propio `os.environ`); y (b) root, o cualquier herramienta de observabilidad/logging que capture variables de entorno de procesos (agentes de APM, `docker inspect` si las variables se pasan como `-e` a un contenedor, paneles de Railway/Render que muestren variables de entorno en su UI de logs — no verificado en este ciclo si la plataforma de despliegue real expone esto).
- **Logs de aplicación:** revisado `main.py` y `sandbox.py` — ningún `logger.info`/`logger.warning`/`logger.error` interpola `password` o el valor de `CC_WORKER_PASSWORD`. El único logging relacionado con credenciales es el mensaje de deprecación (que no incluye el valor, solo advierte del uso del argumento) y el log de login fallido (`main.py` línea 51, que registra `response.text` del servidor — no el password enviado). Confirmado: no hay fuga por logging en el código de este ciclo.
- **Historial de shell:** eliminado para el flujo recomendado (variable de entorno exportada, no pasada como argumento del comando ejecutado) — pero si el operador exporta la variable con `export CC_WORKER_PASSWORD=secret123` directamente en la shell interactiva (en vez de en un archivo `.env` cargado aparte), el valor literal sí queda en `~/.bash_history` igual que antes — el problema se traslada, no desaparece del todo, si el hábito operativo no cambia junto con el código. Esto es responsabilidad operativa, no un defecto del diseño, pero vale la pena señalarlo porque "usar variables de entorno" no es automáticamente seguro si se configuran de forma descuidada.

**Veredicto:** SEC-20 (contraseña del worker en CLI) queda **mitigado para el camino recomendado, no eliminado en su totalidad** — el camino deprecado sigue existiendo y sigue siendo tan inseguro como antes para quien lo use. La superficie de `/proc/<pid>/environ` es estrictamente menor que la de `ps aux`/`cmdline`, pero no es cero, y el hallazgo nuevo más relevante de este ciclo (SEC-37) es que el propio mecanismo de sandboxing diseñado para contener un plugin comprometido le entrega, sin necesitarla, la contraseña que SEC-20 intentaba proteger.

### 4. SEC-36 y el nuevo rate limiting de `/work/claim` — ¿interacción?

**Verificado que no hay conflicto ni refuerzo mutuo directo a nivel de código — son mecanismos ortogonales que operan en capas distintas:**

- El rate limit (`rate_limit_by_provider`, ejecutado como dependencia de FastAPI **antes** del cuerpo del endpoint) y la exclusión por `abandoned_by` (dentro de la transacción SQL de `claim_chunks_atomic`, `compute_queries.py` líneas 260-297) usan conexiones psycopg2 completamente independientes y no comparten transacción ni estado. Uno no puede desactivar ni reforzar al otro a nivel de lógica: pasar el rate limit no exime del filtro `NOT (%(provider_id)s = ANY(c.abandoned_by))`, y estar excluido por `abandoned_by` no consume ni libera cuota de rate limit de forma especial (una petición que no reclama nada por estar todo excluido igualmente cuenta como una petición contra el límite de 30/60s, igual que cualquier otra).
- **Única interacción real, y es de liveness, no de seguridad:** el barrido de reclamo perezoso por TTL (sentencia 1 de `claim_chunks_atomic`, la que resuelve SEC-22 y alimenta `abandoned_by` para SEC-36) solo se ejecuta cuando el código del endpoint llega a correr — es decir, cuando una petición supera el rate limit. Si, en un sistema con muy pocos workers activos, el único proveedor que llama a `/work/claim` estuviera bloqueado por su propio rate limit (escenario improbable: 30/60s es ampliamente superior al ritmo de polling por defecto del worker, ~12/min con `--interval 5`), el barrido de TTL para TODOS los chunks del sistema se retrasaría hasta que ese proveedor (o cualquier otro) vuelva a tener cuota disponible. No es un problema de seguridad — ningún chunk queda permanentemente huérfano por esto, solo se retrasa el barrido — pero es una dependencia de liveness entre ambos mecanismos que no existía antes de este ciclo (antes del rate limit, cualquier llamada a `/work/claim` disparaba el barrido sin condición previa).
- **No se detectó ninguna forma de que el rate limiting debilite la mitigación de SEC-36**, ni de que la exclusión por `abandoned_by` interfiera con el conteo del rate limit. Ambos hallazgos comparten causa raíz de fondo (`/work/claim` sin control de identidad más allá del JWT, ya señalado en el handoff de la sección anterior de este documento), pero no interactúan de forma que uno reabra o enmascare al otro.

### 5. Actualización de estado — SEC-21 y SEC-23

**SEC-21 (sin rate limiting en `/work/claim`) → PARCIALMENTE RESUELTO** (no RESUELTO). Se implementó exactamente lo que el hallazgo original pedía (rate limiting por `provider_id`) y **cierra por completo el escenario de ataque descrito textualmente en el hallazgo original** (un único proveedor acaparando todos los chunks disponibles en bucle cerrado). No cierra el problema en su forma más general: el límite es por cuenta, y crear cuentas sigue siendo gratuito y solo acotado por un rate limit de IP evadible por rotación — de modo que el acaparamiento agregado por varias cuentas del mismo atacante sigue siendo posible (cuantificado arriba en el punto 1: hasta ~6000 chunks/min agregados con un esfuerzo de solo 4 IPs distintas). Esto no es una crítica al trabajo de este ciclo — es exactamente lo que el propio diseño (`docs/04-arquitectura.md` §15.4.3) admite como alcance deliberadamente parcial.

**SEC-23 (Sybil attack en consenso) → SIN CAMBIOS, sigue PENDIENTE.** Las dos medidas de este ciclo (normalización de email a minúsculas + rate limit de registro) actúan exclusivamente sobre el costo de **crear cuentas**, no sobre el problema de fondo que describe SEC-23: dos cuentas distintas controladas por la misma persona que se validan mutuamente en el consenso de `POST /work/{chunk_id}/submit`. Verificado leyendo `consensus_service.py` de nuevo en este ciclo: no hay ningún cambio en la lógica de consenso ni en la verificación de identidad de los proveedores que entregan resultados — el único requisito sigue siendo `UNIQUE (chunk_id, provider_id)`, exactamente como cuando se documentó SEC-23 originalmente. La normalización de email cierra una brecha trivial adyacente (`Ana@Example.com` vs `ana@example.com` como "dos cuentas" por accidente de formato, no por intención de ataque) pero no toca el vector Sybil deliberado. El propio diseño de este ciclo es explícito y honesto al respecto (§15.4.3: "el problema de fondo... sigue exactamente igual de vigente") y esta auditoría lo confirma sin matices: **SEC-23 no ha recibido ninguna mitigación de fondo en este ciclo.**

### Hallazgos nuevos de este ciclo

| ID | Severidad | Título |
|----|-----------|--------|
| SEC-37 | MEDIO | El subproceso "sandboxed" del worker hereda el entorno completo del proceso padre, exponiendo `CC_WORKER_PASSWORD`/`CC_WORKER_EMAIL` sin necesidad |
| SEC-38 | BAJO | `RLIMIT_AS=512MB` no verificado empíricamente contra el uso real de memoria virtual de `polars`/`pyarrow` — riesgo de falsos positivos que presionen a relajar el límite |

---

### SEC-37 — Subproceso sandbox hereda el entorno completo del worker, exponiendo credenciales sin necesidad

**Severidad: MEDIO**
**CWE:** CWE-200 (Exposure of Sensitive Information), CWE-668 (Exposure of Resource to Wrong Sphere)
**Archivo:** `backend/app/worker/sandbox.py` línea 78 (`ctx.Process(target=_child_entrypoint, args=(...))`, sin parámetro `env=`)

**Descripción:** el subproceso creado por `run_chunk_sandboxed` para ejecutar `plugin.process(payload)` hereda automáticamente todas las variables de entorno del proceso worker padre, incluidas `CC_WORKER_PASSWORD` y `CC_WORKER_EMAIL` (§15.2). El plugin no necesita estas variables — la autenticación ya ocurrió en `login()` antes de que exista ningún chunk que procesar — por lo que su presencia dentro del subproceso es una exposición evitable, no una necesidad funcional.

**Vector de ataque:** si un futuro plugin (o una vulnerabilidad de deserialización/ejecución en una dependencia como `polars`) permitiera ejecutar código arbitrario dentro del subproceso, ese código tendría acceso directo a `os.environ["CC_WORKER_PASSWORD"]` sin necesitar ningún privilegio adicional ni leer `/proc/<pid>/environ` — ya está en su propio espacio de proceso. Con esa contraseña, el atacante obtiene acceso completo a la cuenta de proveedor asociada (reclamar chunks, enviar resultados fraudulentos, cobrar recompensas), exactamente el mismo impacto que SEC-20 buscaba prevenir.

**Impacto actual:** BAJO — el plugin `data_processing` no tiene RCE conocida hoy (mismo análisis que SEC-29). **Impacto futuro:** relevante en el mismo momento en que SEC-29 lo señala como crítico (primer plugin con capacidad de ejecutar código no confiable).

**Mitigación concreta:**
```python
# sandbox.py — construir un entorno mínimo explícito para el hijo
_CHILD_ENV_ALLOWLIST = ("PATH", "LANG", "LC_ALL")  # nada de CC_WORKER_*, SUPABASE_*, etc.

def run_chunk_sandboxed(job_type: str, payload: dict[str, Any]) -> tuple[dict, int]:
    ...
    child_env = {k: v for k, v in os.environ.items() if k in _CHILD_ENV_ALLOWLIST}
    ctx = mp.get_context("spawn")
    # multiprocessing no expone un parámetro `env=` directo en Process(); requiere
    # envolver _child_entrypoint para hacer os.environ.clear() + update(child_env)
    # como primera línea dentro del propio hijo, antes de cualquier import de plugin.
```
Nota de implementación: `multiprocessing.Process` no acepta un parámetro `env=` nativo (a diferencia de `subprocess.Popen`) — el hijo ya arrancó heredando el entorno del padre en el momento del `spawn`. La mitigación real debe aplicarse como primera instrucción de `_child_entrypoint`, limpiando `os.environ` a una allowlist mínima antes de importar `app.worker.plugins`.

**Estado actual:** NUEVO — introducido (o, más precisamente, no evitado) por el diseño de sandboxing de este ciclo. No bloqueante para el alcance actual (plugin sin RCE), pero debe resolverse antes o junto con la adición del primer plugin que invoque binarios externos, en el mismo momento en que SEC-29 exige revisar el aislamiento completo.

---

### SEC-38 — Límite de memoria virtual del sandbox (`RLIMIT_AS=512MB`) no verificado empíricamente

**Severidad: BAJO**
**CWE:** CWE-1284 (Improper Validation of Specified Quantity in Input) — aplicado aquí a la validación del propio límite de contención, no a datos de usuario
**Archivo:** `backend/app/worker/sandbox.py` línea 19 (`CHUNK_MEMORY_LIMIT_MB = 512`), línea 44-45 (`resource.setrlimit(resource.RLIMIT_AS, ...)`)

**Descripción:** `RLIMIT_AS` acota memoria virtual direccionable, no memoria residente física. Los allocators de Rust/Arrow en los que se apoya `polars` son conocidos, en general, por reservar regiones de memoria virtual más amplias de lo que el conjunto de datos activo requeriría (arenas de allocator, mapeo de bibliotecas compartidas grandes). No se pudo reproducir en este ciclo de auditoría contra un entorno Linux real con `polars` instalado si 512 MB es suficiente para el caso legítimo (chunks de ~500 filas, según `CHUNK_SIZE` de `compute_service.py`) sin disparar el límite por reserva de memoria virtual no relacionada con el tamaño real de los datos.

**Impacto si se confirmara el riesgo:** chunks legítimos fallarían con `"chunk excedió el límite de memoria del subproceso"` o con el subproceso terminado por `SIGKILL` (código de salida anómalo) sin que haya habido ningún payload malicioso — un falso positivo de seguridad que degradaría el servicio. La respuesta operativa más probable ante ese síntoma sería subir el límite (perdiendo margen de contención real) o desactivarlo, debilitando silenciosamente la única barrera de memoria que introduce este ciclo.

**Mitigación concreta:** antes de depender de este límite en producción, ejecutar el plugin real dentro del sandbox contra el `deploy/backend.Dockerfile` (o una imagen equivalente) con un chunk del tamaño máximo esperado y confirmar que no se acerca al límite de 512 MB de memoria virtual. Si se confirma que el límite es demasiado ajustado, la vía correcta es medir el uso real y fijar el límite con margen sobre esa medición — no eliminarlo.

**Estado actual:** NUEVO — riesgo de verificación pendiente, no confirmado como explotable ni como defecto activo. No bloqueante.

### Resumen ejecutivo de esta sección

| ID | Severidad | Título | Estado |
|----|-----------|--------|--------|
| SEC-20 | ALTO | Contraseña del worker en CLI | **PARCIALMENTE RESUELTO** — camino recomendado (env vars) mitiga `ps aux`; camino `--password` deprecado pero no eliminado sigue igual de expuesto |
| SEC-21 | ALTO | Sin rate limiting en `/work/claim` | **PARCIALMENTE RESUELTO** — cierra el acaparamiento por cuenta única; no cierra el acaparamiento agregado vía múltiples cuentas (mismo origen que SEC-23) |
| SEC-23 | ALTO | Sybil attack en consenso | **SIN CAMBIOS — sigue PENDIENTE.** Este ciclo solo encarece la creación de cuentas, no toca el consenso |
| SEC-29 | MEDIO | Worker sin sandbox completo | **PARCIALMENTE RESUELTO** — aislamiento de proceso (CPU/memoria/tiempo) verificado con test; red/filesystem/entorno siguen sin aislar (ver SEC-37) |
| SEC-37 | MEDIO | Subproceso sandbox hereda entorno completo (credenciales expuestas sin necesidad) | **NUEVO** |
| SEC-38 | BAJO | `RLIMIT_AS` no verificado empíricamente contra `polars`/`pyarrow` | **NUEVO** (riesgo de verificación, no confirmado) |

**Ningún hallazgo de este ciclo es CRÍTICO ni bloqueante para el alcance específico que se propuso resolver.** Las cuatro decisiones del diseño (§15.1-§15.4) están implementadas fielmente y cierran los escenarios de ataque más baratos y directos que motivaron SEC-20/SEC-21/SEC-29 en su formulación original. Ninguna de ellas, ni individualmente ni en conjunto, resuelve el problema de fondo de identidad/Sybil (SEC-23), que sigue siendo el riesgo ALTO más importante sin mitigar de toda la plataforma antes de operar con clientes/jobs de valor económico real — tal como el propio diseño de este ciclo advierte con honestidad en su §15.4.3 y §15.5.

### Handoff para DevOps / Product Owner

1. **No bloqueante para continuar operando en el alcance actual (demo/piloto controlado).** Las cuatro mitigaciones son mejoras reales y verificadas, no placebos — despliegan sin riesgo de regresión funcional (tests unitarios existentes cubren la lógica nueva).
2. **Antes de abrir la plataforma a proveedores no confiables/anónimos a gran escala:** SEC-23 (Sybil de fondo) sigue siendo el hueco más importante. El acaparamiento agregado descrito en el punto 1 de esta sección (~6000 chunks/min con 4 IPs) es la manifestación concreta y cuantificada de por qué SEC-21 no puede considerarse cerrado sin resolver SEC-23 primero, o en el mismo esfuerzo.
3. **SEC-37 (nuevo, MEDIO):** de bajo costo de arreglo (limpiar `os.environ` al inicio de `_child_entrypoint` antes de importar el plugin) — recomendado resolverlo en el mismo ciclo en que se toque de nuevo `sandbox.py`, y obligatorio antes de añadir cualquier plugin que no sea `data_processing`.
4. **SEC-38 (nuevo, BAJO):** verificar empíricamente `RLIMIT_AS=512MB` contra el entorno Linux real de despliegue con un chunk de tamaño máximo antes de confiar en el valor sin más pruebas. Coste de verificación bajo (una ejecución manual), no requiere cambio de diseño si se confirma que el margen es suficiente.
5. **Camino `--email`/`--password` del worker (parte de SEC-20):** sigue existiendo por compatibilidad deliberada. Recomendado fijar una fecha de eliminación real (no indefinida) una vez que DevOps confirme que ningún script de despliegue externo al repo todavía lo usa — de lo contrario el riesgo original de SEC-20 persiste indefinidamente para cualquier despliegue que no haya migrado.
6. **Nada de esta sección requiere revertir ni pausar el trabajo ya desplegado** (`migrations/008_rate_limiting.sql`, cambios de `sandbox.py`/`main.py`/routers) — todos los hallazgos nuevos son incrementales y no reabren ningún vector que el ciclo pretendía cerrar.
