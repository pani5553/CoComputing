...
|------------|-------------|
| ~~Sin sandboxing del worker~~ | **PARCIALMENTE RESUELTO 2026-07-09.** Cada chunk se procesa en un subproceso aislado (`multiprocessing`, `spawn`) con timeout duro de 30s y limites de CPU/memoria (`RLIMIT_CPU`/`RLIMIT_AS`) — un chunk colgado o que agota memoria ya no tumba el proceso worker completo (`backend/app/worker/sandbox.py`). Sigue sin haber aislamiento de red, filesystem ni namespaces de SO: el subproceso puede abrir conexiones salientes arbitrarias y comparte filesystem con el worker. Ver `docs/04-arquitectura.md` §15.3 y `docs/06-security.md` (auditoria 2026-07-09, SEC-29 → PARCIALMENTE RESUELTO). |
...
| ~~Sin rate limiting en `/work/claim`~~ | **PARCIALMENTE RESUELTO 2026-07-09.** `POST /work/claim` (30/min) y `POST /work/{chunk_id}/submit` (60/min) tienen rate limiting por `provider_id`, respaldado por Postgres (`migrations/008_rate_limiting.sql`, ya aplicada a produccion). Cierra el acaparamiento por una unica cuenta en bucle cerrado. NO cierra el acaparamiento agregado usando varias cuentas del mismo atacante (crear cuentas sigue siendo gratis y solo acotado por rate limit de IP, evadible por rotacion — ver fila de Sybil abajo). Ver `docs/04-arquitectura.md` §15.1 y `docs/06-security.md` (SEC-21 → PARCIALMENTE RESUELTO). |
| ~~Credenciales del worker en linea de comandos~~ | **PARCIALMENTE RESUELTO 2026-07-09.** Metodo recomendado: variables de entorno `CC_WORKER_EMAIL`/`CC_WORKER_PASSWORD`, que ya no aparecen en `ps aux` ni en el historial de shell. Los argumentos `--email`/`--password` se mantienen por compatibilidad pero quedan **en desuso** (advertencia de deprecacion en cada arranque) — quien los siga usando sigue tan expuesto como antes. El propio subproceso sandboxed hereda igualmente estas variables del entorno del worker. Ver `docs/04-arquitectura.md` §15.2 y `docs/06-security.md` (SEC-20 → PARCIALMENTE RESUELTO). |
| ~~Sin deteccion de ataques Sybil~~ | **PARCIALMENTE MITIGADO 2026-07-09 (solo en el registro, no en el consenso).** `POST /auth/register` normaliza el email a minusculas y añade rate limiting (5 cuentas/hora/IP), lo que encarece la creacion masiva de cuentas desde un mismo origen. El problema de fondo que describe SEC-23 (varias cuentas reales controladas por la misma persona validandose mutuamente en el consenso de `/work/{chunk_id}/submit`) sigue sin ninguna mitigacion: `UNIQUE (chunk_id, provider_id)` sigue siendo la unica proteccion, igual que antes de este ciclo. Requiere verificacion de email real con un servicio externo o un mecanismo de identidad mas fuerte, ninguno de los cuales existe hoy. Ver `docs/04-arquitectura.md` §15.4 y `docs/06-security.md` (SEC-23 → SIN CAMBIOS, sigue PENDIENTE). |
...
# Co-Computing — Cierre de ciclo: Endurecimiento de Seguridad Pre-Lanzamiento (Rate Limiting, Credenciales del Worker, Sandboxing y Mitigacion Basica de Sybil)

**Fecha:** 2026-07-09
**Autor:** Technical Writer
**Version del producto:** 1.1.0 (sin incremento de version — ciclo de endurecimiento de seguridad, no una feature de producto)
**Referencia:** `docs/04-arquitectura.md` §15, `docs/05-review.md` (seccion fechada 2026-07-09), `docs/06-security.md` (secciones fechadas 2026-07-09)

## Que se entrego

Encargo explicito del Product Owner tras una auditoria de "listo para lanzamiento publico": endurecer 4 puntos de seguridad ya senalados en auditorias anteriores (SEC-02/SEC-21, SEC-20, SEC-29, SEC-23), acotados al alcance real de la infraestructura actual (un unico proceso logico FastAPI/Uvicorn `--workers 2`, Supabase Postgres como unico almacen compartido — sin Redis, sin servicio externo de email).

**1. Rate limiting compartido via Postgres.** Cubre `POST /auth/login` (10/min/IP), `POST /auth/register` (5/hora/IP), `POST /work/claim` (30/min/`provider_id`) y `POST /work/{chunk_id}/submit` (60/min/`provider_id`). El contador vive en la nueva tabla `rate_limit_counters` (`migrations/008_rate_limiting.sql`, ya aplicada a produccion) precisamente porque `--workers 2` invalida cualquier limitador basado en memoria de proceso (dos procesos Uvicorn no comparten heap). Cierra SEC-21 **parcialmente**: bloquea de forma efectiva a un unico actor con una unica cuenta/IP en bucle cerrado, pero no impide el acaparamiento agregado de un atacante con varias cuentas y varias IPs (la creacion de cuentas nuevas solo esta acotada por el rate limit de registro, evadible por rotacion de IP). Diseno completo en `docs/04-arquitectura.md` §15.1; verificado sin hallazgos bloqueantes por Code Reviewer y Security Auditor.

**2. Credenciales del worker por variable de entorno.** `CC_WORKER_EMAIL`/`CC_WORKER_PASSWORD` son ahora el metodo recomendado; `--email`/`--password` pasan a opcionales y emiten advertencia de deprecacion en cada arranque si se usan sin la variable equivalente. `scripts/run_workers.sh` ya no pasa `--password` en ningun caso. El camino deprecado no se elimino (compatibilidad hacia atras deliberada) y sigue siendo tan inseguro como antes para quien lo use.

**3. Sandboxing practico del worker.** Cada chunk se procesa en un subproceso aislado (`multiprocessing`, `spawn`) con timeout duro de pared de 30 segundos y limites de CPU (`RLIMIT_CPU`, 25s) y memoria virtual (`RLIMIT_AS`, 512 MB) — un payload que cuelga o agota memoria ya no tumba el proceso worker completo, solo ese chunk. **Explicitamente NO es aislamiento completo:** sin restriccion de red saliente, sin aislamiento de filesystem, sin namespaces de sistema operativo — eso sigue pendiente para cuando se añada el primer plugin que invoque un binario externo.

**4. Mitigacion basica de Sybil (SEC-23) en el registro.** `RegisterRequest`/`LoginRequest` normalizan el email a minusculas antes de comparar/guardar (cierra la brecha trivial de `Ana@Example.com` vs `ana@example.com` como "dos cuentas" por accidente de formato) y `POST /auth/register` queda cubierto por el mismo rate limit de 5/hora/IP del punto 1. **No resuelve Sybil de fondo:** el problema real que describe SEC-23 — dos cuentas distintas controladas por la misma persona validandose mutuamente en el consenso de `/work/{chunk_id}/submit` — sigue exactamente igual de vigente. La unica proteccion en el consenso sigue siendo `UNIQUE (chunk_id, provider_id)`, sin cambios. Requiere verificacion de email real con un servicio externo (SendGrid/SES/Postmark/Resend) o un mecanismo de identidad mas fuerte (KYC, fingerprint de hardware, proof-of-work) — ninguno existe hoy ni se diseño en este ciclo.

**Ademas, en el mismo ciclo:**

- **Bug de login case-sensitive (encontrado por QA), corregido.** El email se normaliza a minusculas tanto en registro como en login. Se verifico que 0 cuentas reales existentes se veian afectadas por la falta de una migracion de backfill (mismo patron de verificacion ya usado para el TTL de chunks en el ciclo anterior), asi que no hizo falta una migracion `009_normalize_provider_emails.sql` adicional.
- **Bug cronico del BOM en `.env`, corregido**, junto con la causa raiz de por que `CORS_ORIGINS` rompia el arranque de `Settings()`.
- **4 hallazgos adicionales del Reviewer/Security, cerrados:** EN-CRIT-01 (riesgo de bloqueo de cuentas existentes por la normalizacion de email — cerrado tras confirmar 0 cuentas afectadas), SEC-37 (el subproceso del sandbox heredaba el entorno completo del worker, exponiendo `CC_WORKER_PASSWORD` sin necesidad — cerrado limpiando el entorno del hijo a una allowlist minima antes de importar el plugin), EN-MAYOR-01 (`extra="ignore"` en `Settings` ocultaba typos de configuracion en variables con valor por defecto como `ENVIRONMENT` — cerrado anadiendo aviso explicito de campos extra ignorados) y EN-MAYOR-02 (rate limiting sin manejo de errores anadia una dependencia fragil de psycopg2 a `/auth/login`/`/auth/register` — cerrado con `try/except` y politica fail-open).

## Lo que queda explicitamente fuera de este ciclo (o parcialmente resuelto)

- **SEC-21 (sin rate limiting en `/work/claim`) → PARCIALMENTE RESUELTO.** Cierra el escenario original (una cuenta, un origen). No cierra el acaparamiento agregado por multiples cuentas/IPs del mismo atacante — cuantificado por el Security Auditor en ~6000 chunks/min agregados con un esfuerzo de solo 4 IPs distintas.
- **SEC-23 (Sybil de fondo en el consenso) → SIGUE PENDIENTE, sin cambios en el consenso.** Es, segun la propia auditoria de este ciclo, "el riesgo ALTO mas importante sin mitigar de toda la plataforma antes de operar con clientes/jobs de valor economico real".
- **Sandboxing completo** (contenedores con seccomp/AppArmor, aislamiento de red y filesystem) sigue fuera de alcance — se revisara cuando se añada el primer plugin que invoque un binario externo.
- **Verificacion de email real** sigue sin diseñarse — requiere decision de producto y cuenta de un servicio externo (SendGrid/SES/Postmark/Resend o equivalente), igual que en su momento Vercel/Railway/Supabase.
- **EN-MENOR-01** (conexion psycopg2 de `check_rate_limit` no se cierra explicitamente — funciona hoy por refcounting de CPython, fragil ante refactors futuros) y **EN-MENOR-02** (el unico test de timeout del sandbox no ejercita un bucle infinito genuino, solo el arranque lento del subproceso) quedan como deuda de test, no bloqueantes.
- **SEC-38** (`RLIMIT_AS=512MB` no verificado empiricamente contra el uso real de memoria virtual de `polars`/`pyarrow`) queda como riesgo de verificacion pendiente, no confirmado como defecto.
- **El gate de cobertura de tests** (80% requerido, 60.33% real) sigue siendo deuda preexistente de ciclos anteriores (modulos de cliente/computo/wallet), no introducida ni agravada por este ciclo — confirmado de forma independiente por el Code Reviewer.
- Todo lo relacionado con pagos reales (Stripe/SEPA) y aspectos legales de una auditoria de lanzamiento publico mas amplia (terminos de servicio, privacidad, cumplimiento) queda fuera del alcance tecnico de este ciclo.

## Documentacion entregada en este ciclo

| Fichero | Descripcion |
|---------|-------------|
| `docs/04-arquitectura.md` | Seccion 15 nueva: diseño de las 4 mitigaciones, con handoffs a Database Engineer y Backend Dev |
| `docs/05-review.md` | Revision del ciclo (seccion 2026-07-09): 1 critico (EN-CRIT-01, luego cerrado), 2 mayores, 2 menores, 3 informativos |
| `docs/06-security.md` | Auditoria del ciclo (seccion 2026-07-09): SEC-20/SEC-21/SEC-29 → parcialmente resueltos, SEC-23 sin cambios, SEC-37 y SEC-38 nuevos |
| `README.md` | Actualizada la seccion "Computo Real Distribuido" (metodo recomendado de credenciales del worker) y la nota de seguridad del worker (alcance real del sandboxing) |
| `docs/07-manual.md` | Actualizadas las instrucciones de arranque del worker (variables de entorno como metodo recomendado, `--email`/`--password` marcados en desuso) |
| `docs/07-entrega.md` | Actualizada la tabla "Limitaciones conocidas del MVP" (4 filas); esta entrada de cierre de ciclo |
